...
#[test]
fn dies_year_13() -> Result<()> {
    Command::cargo_bin(PRG)?
        .arg("10000")
        .assert()
        .failure()
        .stderr(predicate::str::contains(
        "error: invalid value '10000' for '[YEAR]': year \"10000\" not in the range 1 through 9999",
    ));
    Ok(())
}
...
fn dies_invalid_year() -> Result<()> {
    Command::cargo_bin(PRG)?
        ...
        .failure()
        .stderr(predicate::str::contains(
            "error: invalid value 'foo' for '[YEAR]': Invalid integer \"foo\"",
        ));
    Ok(())
}
...
fn dies_month_0() -> Result<()> {
    Command::cargo_bin(PRG)?
        ...
        .failure()
        .stderr(predicate::str::contains(
            "error: invalid value '0' for '--month <MONTH>': month \"0\" not in the range 1 through 12",
        ));
    Ok(())
}
...
fn dies_month_13() -> Result<()> {
    Command::cargo_bin(PRG)?
        ...
        .failure()
        .stderr(predicate::str::contains(
            "error: invalid value '13' for '--month <MONTH>': month \"13\" not in the range 1 through 12",
        ));
    Ok(())
}
...
fn dies_invalid_month() -> Result<()> {
    Command::cargo_bin(PRG)?
        ...
        .failure()
        .stderr(predicate::str::contains(
            "error: invalid value 'foo' for '--month <MONTH>': Invalid month \"foo\"",
        ));
    Ok(())
}
...
fn dies_y_and_month() -> Result<()> {
    let expected = "error: the argument '--month <MONTH>' cannot be used with '--year'";
    Command::cargo_bin(PRG)?
        .args(["-m", "1", "-y"])
        .assert()
        .failure()
...
fn dies_y_and_year() -> Result<()> {
    let expected = "error: the argument '--year' cannot be used with '[YEAR]'";
    Command::cargo_bin(PRG)?
        .args(["-y", "2000"])
        .assert()
        .failure()
